...
As for problem 1, First we plot the price data and found that they show non-stationarity in the sense of mean. We therefore utilize \Predictor to predict the prices of gold and bitcoin. Our choice of parameters is ARIMA(4,1,4) for both bitcoin and gold. The predictor shows good capability to accurately predict the price of future.

To find the best timing to sell and buy the two assets, we first rate them with the our well-designed rating system by various factors like changes in value, moving averages, and biases. Then from the factors our model further calculates our indicator for risk and trend. From these two indicators, our main factor can give us enough information to make trading decisions.

To make trading decisions, we furthermore set thresholds based on our observations and experiments in our data and we balance our portfolio by the Markowiz's asset portfolio model.
