...
There are some missing value in the provided data of gold prices, as is shown in Figure \ref{figure:missing}.

\begin{figure}[H]
	\centering{\includegraphics[width=17.0cm]{figures/missing.png}}
	\caption{ Data-dense view }
...
\begin{figure}[H]
	\caption{Filling the missing data around 12/22/2017 and 12/29/2017 }
	\label{figure:fill_missing}
	\centering
	\begin{minipage}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=8cm]{figures/example_missing.png}
		\caption{Before}
	\end{minipage}
	\begin{minipage}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=8cm]{figures/fill_missing.png}
		\caption{After}
	\end{minipage}
\end{figure}

 To make up the them, moving median method is used, where the length of window is 5. For each missing value $v_0$, we chose a window $$v_{-2},v_{-1},v_0,v_1,v_2$$
 Where $v_{-2}$, $v_{-1}$ are the predecessors of $v_0$ and $v_1$,$v_2$ are the successors of $v_0$.
 Then, we let the value of $v_0$ be the \textit{median} of $\{v_{-2},v_{-1},v_1,v_2\}$. 
 
 
 The method is commonly used when only a few values are missing, which is fast and fit in the trend of the provided data well, as is shown in the Figure \ref{figure:fill_missing}. 

\subsection{Feature Engineering}
Feature engineering, also known as feature extraction, extracts features from raw data using domain knowledge. By feature engineering, we will improve the quality of result from the following machine learning process. Here are techniques we used for feature engineering:

\subsubsection{Fractionally Differentiated Features}


\subsection{\Predictor Tuning}

\subsubsection{Multiple Features or Single Feature}

\subsubsection{Finite Difference or Not}

\subsubsection{Long Difference Period or Short}

\subsubsection{Long Retrospective Cycle or Short}

\subsection{Position Management and Risk Management}

\subsubsection{Kelly Formula}

\subsubsection{Bolling Filter}
